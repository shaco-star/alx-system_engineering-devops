...
exec { 'increase-soft-file-limit-for-holberton-user':
  command => 'grep -q "^holberton soft" /etc/security/limits.conf &&
sed -i "/holberton soft/s/4/50000/" /etc/security/limits.conf || echo "holberton soft nofile 50000" >> /etc/security/limits.conf',
  path    => '/usr/local/bin/:/bin/',
  require => User['holberton'],
}
